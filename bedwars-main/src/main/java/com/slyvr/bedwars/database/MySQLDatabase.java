...
public final class MySQLDatabase implements Database {
    ...
    public @NotNull UserPrestige getUserPrestige(@NotNull UUID uuid) {
        ...
        try (PreparedStatement statement = connection.prepareStatement("SELECT `Prestige` FROM `user_level` WHERE UUID = ?")) {
            ...
            ResultSet result_set = statement.executeQuery();
            if (result_set.next())
                return UserPrestige.getByName(result_set.getString("Prestige"), Bedwars.getInstance().getDefaultPrestige());

        } catch (Exception ignored) {

        }

        return Bedwars.getInstance().getDefaultPrestige();
    }
    ...
    public boolean saveUserStatistics(@NotNull OfflineUser user, @NotNull GameMode mode) {
        ...
        try (PreparedStatement statement = connection.prepareStatement(builder)) {
            ...
            Bedwars.getBedwarsLogger().info("Could not save user '" + user.getUniqueId() + "' " + mode.getName() + " statistics!");
            return false;
        }
    ...
    public boolean saveUserQuickBuy(@NotNull OfflineUser user, @NotNull GameMode mode) {
        ...
        try (PreparedStatement statement = connection.prepareStatement(builder)) {
            ...
            Bedwars.getBedwarsLogger().info("Could not save user '" + user.getUniqueId() + "' " + mode.getName() + " quick-buy!");
            return false;
        }
    ...
    public boolean saveUserLevel(@NotNull OfflineUser user) {
        ...
        try (PreparedStatement statement = connection.prepareStatement(builder)) {
            ...
            Bedwars.getBedwarsLogger().info("Could not save user '" + user.getUniqueId() + "' level!");
            return false;
        }
    ...
    public boolean saveUserWallet(@NotNull OfflineUser user) {
        ...
        try (PreparedStatement statement = connection.prepareStatement(builder)) {
            ...
            Bedwars.getBedwarsLogger().info("Could not save user '" + user.getUniqueId() + "' wallet!");
            return false;
        }
    ...
    public boolean saveUserLanguage(@NotNull OfflineUser user) {
        ...
        try (PreparedStatement statement = connection.prepareStatement(builder)) {
            ...
            Bedwars.getBedwarsLogger().info("Could not save user '" + user.getUniqueId() + "' language!");
            return false;
        }
